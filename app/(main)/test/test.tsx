...
export default function Test() {
  ...
  const handleRequestTest = async () => {
    ...
    try {
      const res = await requestForTest({ walletAddress })
      setTestLink(res.data.data[0].url)
      setHasTest(true)
    } catch (err) {
      console.error(err)
      alert("Failed to request test link")
    }
  }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const yourTest = await getYourTest({ walletAddress })
        setTestLink(yourTest.data.data.url)
        setHasTest(true)
        setLoading(false)
      } catch (err) {
        console.error(err)
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#001740] flex items-center justify-center text-[#fbc816] text-xl">
      Loading...
    </div>
  )
  ...
  return (
    <main className="min-h-screen bg-[#001740] text-white">
      <section className="flex flex-col items-center justify-center px-6 py-24 text-center">
        <h1 className="text-4xl font-extrabold mb-8 text-[#fbc816]">Knowledge Test</h1>

        {!walletAddress && (
          <p className="text-red-400 mb-6">Connect your wallet to access the test.</p>
        )}
        ...
        {walletAddress && verified && (
          hasTest ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="bg-[#fbc816] text-[#001740] px-8 py-4 rounded-2xl font-bold shadow-xl hover:scale-105 transition"
            >
              Take the Test
            </a>
          ) : (
            <button
              onClick={handleRequestTest}
              className="bg-[#2417d3] hover:bg-[#1f14b0] text-[#fbc816] px-8 py-4 rounded-2xl font-bold shadow-xl hover:scale-105 transition"
            >
              Request Test link
            </button>
          )
